...
OrtStatus* ORT_API_CALL IreeEp::CompileImpl(
  ...
  // Keep files on disk when caching is enabled (save_intermediates or
  // enable_ep_context). Otherwise, temp files are deleted when they go out of
  // scope — the VMFB is read into memory below and doesn't need the file.
  bool cache_artifacts =
      ep->config_.save_intermediates || ep->config_.enable_ep_context;
  if (cache_artifacts) {
    mlir_file.Keep();
    vmfb_file.Keep();
    irpa_file.Keep();
