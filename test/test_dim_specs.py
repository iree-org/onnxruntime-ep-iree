...
def _create_matmul_model(batch_dim="batch", seq_dim="seq", hidden=16):
...
def _create_add_model(batch_dim="batch", feat_a=10, feat_b=20):
...
def test_static_specialization(iree_device):
    """Static dim_specs: matching and non-matching shapes produce correct results."""
    hidden = 16
    model_path, weight_data = _create_matmul_model(hidden=hidden)
    try:
...
def test_divisibility_specialization(iree_device):
...
def test_multi_variant_dispatch(iree_device):
...
def test_parse_errors(iree_device):
    ...
    """
    model_path, _ = _create_matmul_model()
    try:
        bad_inputs = [
...
def test_save_intermediates_static(iree_device):
    ...
    try:
        ...
        ), "should have flow.tensor.tie_shape"
...
def test_save_intermediates_divisibility(iree_device):
    ...
    try:
        ...
        assert (
            "util.assume.int" in mlir_content
        ), "MLIR should contain util.assume.int ops"
...
def test_empty_dim_specs(iree_device):
...
def test_mixed_static_divisibility(iree_device):
...
def test_shared_symbolic_dims(iree_device):
    """Two inputs sharing a symbolic dim must reuse the same canonical assume."""
    feat_a, feat_b = 10, 20
    model_path, weight_data = _create_add_model(feat_a=feat_a, feat_b=feat_b)
    mlir_before = _snapshot_mlir_files()
...
def test_shared_dim_conflict_fallback(iree_device):
    ...
    try:
        ...
        try:
            ...
            assert (
                result2.shape[1] == feat_b
            ), f"unexpected output shape: {result2.shape}"
        except Exception:
...
def test_whitespace_and_divisibility_by_one(iree_device):
...
def test_unknown_dim_spec_rejected(iree_device):
